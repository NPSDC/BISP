...
cv.nb.list <- function(tr.data, train.model.list, folds, genes.list, stages.train, cores = 1)
{
  nb.pred <- mclapply(seq_along(genes.list), function(i)
  {
    if(is.null(genes.list[[i]]) | (length(genes.list[[i]]) == 1))
      return(NULL)
    else
      return(cv.naiveBayes(tr.data = tr.data[, genes.list[[i]]], 

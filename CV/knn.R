...
cv.knn.list <- function(tr.data, folds, genes.list, stages.train, cores = 1)
{
  knn.pred <- mclapply(genes.list, function(genes)
  {
    if(is.null(genes) | (length(genes) == 1))
      return(NULL)
    else
      return(find.best.k(tr.data[, genes], folds, stages.train, cores = cores))
  }, mc.cores = cores)

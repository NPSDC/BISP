...
#' @param genes.list List of different groups of a particular feature selection algorithm
#' @param train.models.list List of training models of shrunken for the different gene sets
...
cv.shrunken <- function(tr.data, folds, genes.list, train.models.list, stages.train, cores = 1)
{
  cv.models <- mclapply(seq_along(genes.list), function(i)
  {
     if(is.null(genes.list[[i]]) | (length(genes.list[[i]]) == 1))
       return(NULL)
     else
      return(pamr.cv(train.models.list[[i]], list(x = t(as.matrix(tr.data[, genes.list[[i]]])),

...
get.cv.model <- function(tr.data, fea.list, folds = 10, stages.train, tr.model, cores = 1)
{
  ...
  cv.model <- foreach(i = 1:length(fea.list)) %dopar%
  {
    ...
    print(fea.name)
    model[['shrunken']] <- cv.shrunken(tr.data = tr.data, folds = folds, genes.list = fea.list[[fea.name]],
    ...
    # cat(nrow(tr.data))
    model[['rf']] <- cv.rf.list(tr.data = tr.data, folds = folds, genes.list = fea.list[[fea.name]],
                                train.model.list = tr.model[[fea.name]]$rf, stages.train = stages.train, cores = cores)
    print('RF completed')
    model[['nb']] <- cv.nb.list(tr.data = tr.data, folds = folds, genes.list = fea.list[[fea.name]],
                                train.model.list = tr.model[[fea.name]]$nb, stages.train = stages.train, cores = cores)
    print('NB completed')
    model[['svm']] <- cv.svm.list(tr.data = tr.data, folds = folds, genes.list = fea.list[[fea.name]],
                                  train.model.list = tr.model[[fea.name]]$svm, stages.train = stages.train, cores = cores)
    print('SVM completed')
    model[['knn']] <- cv.knn.list(tr.data = tr.data, folds = folds, genes.list = fea.list[[fea.name]],
                                                 stages.train = stages.train, cores = cores)
    return(model)
  }
